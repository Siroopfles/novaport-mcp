...
router = APIRouter(prefix="/workspaces/{workspace_id_b64}/system-patterns", tags=["System Patterns"])

@router.post("/", response_model=sp_schema.SystemPatternRead, status_code=status.HTTP_201_CREATED)
def create_system_pattern(workspace_id_b64: str, pattern: sp_schema.SystemPatternCreate, db: Session = Depends(get_db)):
    """Log a new system or architectural pattern."""
    workspace_id = decode_workspace_id(workspace_id_b64)
    return system_pattern_service.create(db=db, workspace_id=workspace_id, pattern=pattern)

@router.get("/", response_model=List[sp_schema.SystemPatternRead])
def read_system_patterns(workspace_id_b64: str, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
def delete_system_pattern(workspace_id_b64: str, pattern_id: int, db: Session = Depends(get_db)):
