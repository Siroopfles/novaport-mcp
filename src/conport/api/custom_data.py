...
def upsert_custom_data(workspace_id_b64: str, data: cd_schema.CustomDataCreate, db: Session = Depends(get_db)):
...
def read_custom_data_by_category(workspace_id_b64: str, category: str, db: Session = Depends(get_db)):
...
def read_custom_data_item(workspace_id_b64: str, category: str, key: str, db: Session = Depends(get_db)):
...
def delete_custom_data_item(workspace_id_b64: str, category: str, key: str, db: Session = Depends(get_db)):
    """Delete a custom data item by its category and key."""
    workspace_id = decode_workspace_id(workspace_id_b64)
    deleted = custom_data_service.delete(db, workspace_id=workspace_id, category=category, key=key)
    if not deleted:
        raise HTTPException(status_code=404, detail="Custom data item not found")
    return