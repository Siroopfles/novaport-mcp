...
@router.post(
    "/", response_model=link_schema.LinkRead, status_code=status.HTTP_201_CREATED
)
def create_link(
    workspace_id_b64: str, link: link_schema.LinkCreate, db: Session = Depends(get_db)
):
...
def read_links_for_item(
    workspace_id_b64: str, item_type: str, item_id: str, db: Session = Depends(get_db)
):
