...
def _get_or_create(db: Session, model):
    ...
    return instance
def get_product_context(db: Session): return _get_or_create(db, models.ProductContext)
def get_active_context(db: Session): return _get_or_create(db, models.ActiveContext)
def update_context(db: Session, instance, update_data: context_schema.ContextUpdate):
    ...
    if update_data.content is not None: new_content = update_data.content
    elif update_data.patch_content is not None:
        for key, value in update_data.patch_content.items():
            if value == "__DELETE__": new_content.pop(key, None)
