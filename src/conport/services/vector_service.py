...
def cleanup_chroma_client(workspace_id: str) -> None:
    ...
    if db_path in _chroma_clients:
        try:
            ...
            time.sleep(
                CHROMA_CLEANUP_DELAY
            )  # Give Windows more time to release handles
            ...
            log.info(
                f"ChromaDB client successfully cleaned up for workspace: {workspace_id} (db_path: {db_path})"
            )
        except Exception as e:
            log.error(
                f"Error cleaning up ChromaDB client for {workspace_id} (db_path: {db_path}): {e}"
            )
...
def get_chroma_client(workspace_id: str) -> Client:
    ...
    if db_path not in _chroma_clients:
        log.info(
            f"Initializing ChromaDB client for workspace: {workspace_id} (db_path: {db_path})"
        )
...
def get_collection(
    workspace_id: str, collection_name: str = "conport_default"
) -> chromadb.Collection:
    ...
    try:
        ...
        try:
            ...
            log.info(
                f"Existing collection '{collection_name}' found for {workspace_id}"
            )
        except Exception as e:
            # If the collection doesn't exist, create a new one
            log.info(
                f"Collection '{collection_name}' not found for {workspace_id}, creating: {str(e)}"
            )
...
def upsert_embedding(
    workspace_id: str, item_id: str, text_to_embed: str, metadata: Dict[str, Any]
) -> None:
...
def delete_embedding(workspace_id: str, item_id: str) -> None:
    ...
    try:
        ...
        log.warning(
            f"Could not delete embedding for {item_id} in workspace {workspace_id} (may not have existed): {e}"
        )
...
def search(
    ...
    for i in range(len(ids)):
        output.append(
            {"id": ids[i], "distance": dist_list[i], "metadata": meta_list[i]}
        )
    return output
