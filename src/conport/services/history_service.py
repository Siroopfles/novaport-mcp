...
def _add_history(target, history_model, change_source):
    ...
    latest_version_stmt = select(history_model.version).order_by(history_model.version.desc()).limit(1)
    latest_version = session.execute(latest_version_stmt).scalar_one_or_none() or 0
    new_history_record = history_model(version=latest_version + 1, content=old_content, change_source=change_source)
