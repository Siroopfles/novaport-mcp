...
def create(db: Session, entry: progress_schema.ProgressEntryCreate, linked_item_type: Optional[str], linked_item_id: Optional[str], link_relationship_type: str) -> models.ProgressEntry:
    ...
    vector_service.upsert_embedding(f"progress_{db_entry.id}", text, metadata)
    ...
    return db_entry
def get(db: Session, entry_id: int) -> models.ProgressEntry | None: return db.query(models.ProgressEntry).filter(models.ProgressEntry.id == entry_id).first()
def get_multi(db: Session, skip: int = 0, limit: int = 100, status: Optional[str] = None, parent_id: Optional[int] = None) -> List[models.ProgressEntry]:
    ...
    if status: query = query.filter(models.ProgressEntry.status == status)
    if parent_id is not None: query = query.filter(models.ProgressEntry.parent_id == parent_id)
...
def update(db: Session, entry_id: int, update_data: progress_schema.ProgressEntryUpdate) -> models.ProgressEntry | None:
    ...
    if db_entry:
        ...
        for key, value in update_dict.items(): setattr(db_entry, key, value)
        db.commit(); db.refresh(db_entry)
    return db_entry
def delete(db: Session, entry_id: int) -> models.ProgressEntry | None:
    db_entry = get(db, entry_id)
    if db_entry: vector_service.delete_embedding(f"progress_{entry_id}"); db.delete(db_entry); db.commit()
    return db_entry