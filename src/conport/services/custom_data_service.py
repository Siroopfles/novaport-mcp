...
def upsert(db: Session, workspace_id: str, data: cd_schema.CustomDataCreate) -> models.CustomData:
    ...
    try:
        ...
        vector_service.upsert_embedding(workspace_id, f"custom_data_{db_data.id}", text, metadata)
    except (TypeError, OverflowError):
        log.warning(f"Could not serialize value for custom_data {db_data.id} for embedding.")
    return db_data
