...
# --- Tool Definities (Workspace-Aware & Async) ---
...
async def semantic_search_conport(
    ...
    # Vector service is not async, but we make the tool async for consistency
    # with the other tools that hit the DB.
...
async def get_conport_schema(
    ...
    """
    # This function itself doesn't need a db session, but is async for consistency.
    tool_functions = [
